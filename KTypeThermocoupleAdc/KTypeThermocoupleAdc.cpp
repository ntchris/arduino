...
float KTypeThermocoupleAdc::convertAdcIntToVoltage( unsigned int adcInt)
{
  ...
  float adcVoltage = 1.0f * AdcRef * (adcInt - 1)  / AdcIntMax + ArduiniAdcMinMv / 1000.0;

  Serial.println( "adcVoltage : "   + String(adcVoltage, 3) );
  return adcVoltage;
}
...
// using lookup table
float KTypeThermocoupleAdc::getTemperatureFromLookupTableAdcInt(unsigned int adcInt)
{
  float temp = 0;
...
int KTypeThermocoupleAdc::valueToLookupTableIndex(int value, int startindex, int endindex)
{
  int index;
...
float KTypeThermocoupleAdc::convertAdcToTempMvPerC(unsigned int adcInt)
{

  const float KTypeMvPerC = 0.04068;
  const float JTypeMvPerC = 0.05362;

  float adcVoltage = KTypeThermocoupleAdc::convertAdcIntToVoltage(adcInt);

  Serial.println("opamp V is " + String(adcVoltage, 3));
  //mV = 1000*V


  float tcOutputMv = adcVoltage * 1000.0 / OpAmpGain;
  //Jtype
...
float KTypeThermocoupleAdc::convertAdcToTempMvPerCPerStage(unsigned int adcInt)
{

  double adcVoltage = KTypeThermocoupleAdc::convertAdcIntToVoltage(adcInt);

  Serial.println("opamp V is " + String(adcVoltage, 3));
  //mV = 1000*V


  double tcOutputMv = adcVoltage * 1000.0 / OpAmpGain;
  ...
  Serial.println("Tc V is " + String(tcOutputMv, 3));
